...
mod switch { 
    ...
    use libnx_rs::libnx::Thread as ThreadHandle;
    ...
    impl Thread {
        pub unsafe fn new<'a>(stack: usize, p: Box<FnBox() + 'a>) -> io::Result<Thread> {
            ...
            libc::svcGetThreadPriority(&mut priority, 0xFFFF8000);

            let mut handle : ThreadHandle = mem::zeroed();//::libctru::threadCreate(Some(thread_func), &*p as *const _ as *mut _,
                        //                        stack_size, priority, -2, false);
            let res = libnx::threadCreate(&mut handle as *mut ThreadHandle, Some(thread_func), &*p as *const _ as *mut libnx::lang_items::c_void, stack_size, priority as i32, -2);

            return if res != 0 {
                Err(io::Error::new(io::ErrorKind::Other, "Out of memory!"))
            } else {
                mem::forget(p); // ownership passed to the new thread
                Ok(Thread { handle: handle })
            };

            extern "C" fn thread_func(start: *mut libnx::lang_items::c_void) {
                unsafe { start_thread(start as *mut u8) }
            }
        }
        ...
        pub fn sleep(dur: Duration) {
            unsafe {
                let nanos = dur.as_secs()
                    .saturating_mul(1_000_000_000)
                    .saturating_add(dur.subsec_nanos() as u64);
                libnx::svcSleepThread(nanos as u64);
            }
        }

        pub fn join(mut self) {
            unsafe {
                let ret = libnx::threadWaitForExit(&mut self.handle as *mut _);
                libnx::threadClose(&mut self.handle as *mut _);
                mem::forget(self);
                debug_assert_eq!(ret, 0);
            }
        }
    ...
    impl Drop for Thread {
        fn drop(&mut self) {
            unsafe { libnx::svcCloseHandle(self.handle.handle) };
        }
    }
